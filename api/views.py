...
def stream_llm_response(prompt: str):
    try:
        with requests.post(OLLAMA_GENERATE_URL, json={
...
def chat_message(request):
    try:
        ...
        if not chat:
            return prepare_failure_response("Chat not found.")

        asset_id = chat.asset.asset_id
        faiss_index_path = f"{ASSET_DIR}/{asset_id}"

        vector_store = FAISS.load_local(faiss_index_path, DocumentEmbedding(), allow_dangerous_deserialization=True)
        ...
        def response_stream():
            ...
            for token in stream_llm_response(prompt):
                answer += token
                yield token
            Conversation.objects.create(chat=chat, user_query=user_query, bot_response=answer)
